...
`git clone https://github.com/ByteLegendQuest/java-calculate-root-of-quadratic-equation`

A project directory named `java-calculate-root-of-quadratic-equation` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-calculate-root-of-quadratic-equation` directory which is created just now.
